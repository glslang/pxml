...
- **Phase A — boundary scan (`src/scan.rs`, single-threaded).** A hand-written
  `memchr`-driven state machine walks the buffer once to find depth-1 element
  boundaries. It builds no tree, decodes no entities, and validates only
  structural well-formedness (depth, matching root end-tag, whitespace-only text
  directly under root). It also captures the shared **prelude** (encoding, root
  name/namespaces, internal-subset `<!ENTITY>` definitions). Output is a
  `ChunkIndex`: a `Range<usize>` per record plus an `Arc<Prelude>`. This is the
  irreducible sequential fraction and is memory-bandwidth bound.
